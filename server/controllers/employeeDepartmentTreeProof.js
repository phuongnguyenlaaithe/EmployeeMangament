const employeeDepartmentTreeProofRouter = require('express').Router();
const { StandardMerkleTree } = require('@openzeppelin/merkle-tree');
const fs = require("fs"); // Import fs module separately

// API: Lấy proof từ Merkle Tree dựa trên hai ID
employeeDepartmentTreeProofRouter.get('/', (req, res) => {
  try {
    // (1) Đọc dữ liệu Merkle Tree từ tệp tree.json
    const treeData = JSON.parse(fs.readFileSync('tree/employeeDepartmentTree.json', 'utf8'));
    ...
    const departmentIdtring = departmentId.toString();
    let proof = null;

    for (const [i, v] of tree.entries()) {
      if (v[0] === employeeIdString && v[1] === departmentIdtring) {
...
module.exports = employeeDepartmentTreeProofRouter;
